#!/usr/bin/env bash
# Thin stub — logic lives in the zero-employee package (`zeo hooks session-start`).
...
[ -z "$ZEO" ] && { echo "zeo not on PATH - orient manually."; exit 0; }
exec "$ZEO" hooks session-start "$@"
