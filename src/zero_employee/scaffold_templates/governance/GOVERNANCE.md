...
Tool-neutral doctrine. Runtime entrypoints (`AGENTS.md`, `CLAUDE.md`, `GEMINI.md`)
point here. Corpus discovery still uses `claude-md/CLAUDE.md` as the marker file.
...
- Seat type names (`zeo-master`, `zeo-sparring`, `zeo-stream`) are constructors, not addresses of running instances. If `zeo relay status` lists a destination instance, send through `zeo relay`; do not spawn another agent of that seat type.

## 1. Operating Rules & Doctrine

- Single Source of Truth: this file (`governance/GOVERNANCE.md`). IDE entrypoints import it via `claude-md/CLAUDE.md`.
...
- Relay messages coordinate live seat instances. SOWs and rulings remain durable organizational truth (`zeo --inbox`).
