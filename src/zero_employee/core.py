...
def _inherited_banner(*, upstream_label: str, sha: str, synced: str) -> str:
    return (
        ...
        f"{upstream_label}: upstream bytes + a fixed transform list (sow-lint --resync-apply).\n"
        ...
        "RE-SYNC DUTY: when upstream doctrine moves, re-run sow-lint --resync-apply in ONE commit.\n"
...
_INDEX_FENCE_OPEN_RULING = "<!-- RULING-INDEX:AUTO — regenerated WHOLE by sow-lint --ruling-index, do not hand-edit -->"
    ...
    *,
    ...
    for attempt in range(retries):
        ...
        body = (
            ...
            f"(stub reserved by `sow-lint --mint sow` — replace this body)\n"
        )
    ...
    *,
    ...
    for attempt in range(retries):
        ...
        body = (
            ...
            f"(stub reserved by `sow-lint --mint ruling` — replace this body)\n"
        )
        if _exclusive_create(dest, body):
...
STATE_FENCE_OPEN = "<!-- STATE:AUTO — regenerated by sow-lint --board, do not hand-edit -->"
...
def kosten(root, stream=None, estimate=None):
    ...
    `sow-lint --session-cost` (transcript / session-costs.jsonl) for that. This function
...
_INDEX_FENCE_OPEN = "<!-- STREAM-INDEX:AUTO — regenerated WHOLE by sow-lint --stream-index, do not hand-edit -->"
...
def check_resolved_by(fm, root, ruling_stems=None):
    ...
    # SIBLING repo. A lexically relative root (Path(".") from `sow-lint --triage .`,
...
def splice_state_zone(existing, zone, title="the fleet"):
    ...
    if existing is None:
        return (
            f"# STATE — {title}\n\n"
            "<!-- ROADMAP: authored by Master. sow-lint never touches anything above the fence. -->\n\n" + zone + "\n"
        )
