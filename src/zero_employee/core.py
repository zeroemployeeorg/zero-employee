...
# RULING-251 s3: the fence matcher keyed on the tool's CURRENT name, so any STATE.md
# written before the sow-lint -> zeo rename (which carries this exact opener with the
# old name instead) read as "open marker missing" — indistinguishable from a genuinely
# malformed fence — and FATALed. Accept EITHER wording on READ; always WRITE the current
# one, so an old-fenced file self-heals to current form the next time it is regenerated
# (RULING-076: regenerated whole, hand-edit self-heals — the same doctrine applied to a
# rename instead of a hand-edit). Add a new alias here, never remove an old one — a file
# fenced under a name that later gets retired again must keep reading.
_STATE_FENCE_OPEN_ALIASES = (
    STATE_FENCE_OPEN,
    "<!-- STATE:AUTO — regenerated by sow-lint --board, do not hand-edit -->",
)


def _find_state_fence_open(existing):
    """Return the position of whichever recognised open-fence wording appears first.

    Tries the CURRENT wording first (the common case), then falls back through
    every retired wording in `_STATE_FENCE_OPEN_ALIASES`. Returns -1 only if NONE
    of them are found — the same "genuinely missing" signal callers already expect
    from `str.find`.
    """
    for alias in _STATE_FENCE_OPEN_ALIASES:
        idx = existing.find(alias)
        if idx != -1:
            return idx, alias
    return -1, None
...
def splice_state_zone(existing, zone, title="the fleet"):
    ...
    o, _matched_open = _find_state_fence_open(existing)
    c = existing.find(STATE_FENCE_CLOSE)
    if o == -1 and c == -1:
    ...
    # `zone` always opens with the CURRENT STATE_FENCE_OPEN (render_state_zone), so
    # splicing over an old-wording fence self-heals it to current wording in this
    # same write — no separate migration step needed (RULING-251 s3).
...
def salvage_state_prefix(existing):
    ...
    if existing is None:
        return None
    open_idx, _matched_open = _find_state_fence_open(existing)
    indices = [i for i in (open_idx, existing.find(STATE_FENCE_CLOSE)) if i != -1]
    if not indices:
        return None
